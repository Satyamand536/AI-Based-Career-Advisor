...
const DOMAINS = [
  ...
  { id: "Node.js",         icon: "🟢",  color: "#f97316" },
  { id: "Python",          icon: "🐍",  color: "#f59e0b" },
  { id: "System Design",   icon: "🏗️",  color: "#f59e0b" },
  { id: "DSA",             icon: "📐",  color: "#f472b6" },
  { id: "Docker & DevOps", icon: "🐳",  color: "#06b6d4" },
  { id: "SQL & Databases", icon: "🗄️",  color: "#fcd34d" },
  { id: "TypeScript",      icon: "🔷",  color: "#fdba74" },
...
export default function SkillGapTests() {
  ...
  const timerColor = timeLeft < 60 ? "#ef4444" : timeLeft < 120 ? "#f59e0b" : "#f97316";
      ...
      <main style={layout.main}>
              ...
              {skillGaps.length > 0 && (
                <div style={{ ...styles.card, position: "sticky", top: 20 }}>
                  ...
                  <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
                    {skillGaps.map((gap, i) => (
                      <div key={i} style={{ ...styles.gapChip, borderColor: gap.priority === "high" ? "#ef4444" : gap.priority === "medium" ? "#f59e0b" : "#6b7280" }}>
                        <div style={{ display: "flex", justifyContent: "space-between", width: "100%" }}>
                            <span style={{ fontSize: 13, fontWeight: 700, color: gap.priority === "high" ? "#f87171" : gap.priority === "medium" ? "#fbbf24" : "#4b5563" }}>
                              {gap.skill}
                            </span>
                  ...
                  <div style={{ marginTop: 16, fontSize: 12, color: "#6b7280", background: "rgba(255,255,255,0.03)", padding: 12, borderRadius: 8 }}>
              ...
              <div style={styles.domainGrid}>
                {DOMAINS.map(domain => (
                  <button
                    ...
                    style={{
                      ...styles.domainBtn,
                      borderColor: selectedDomain === domain.id ? domain.color : "rgba(17,24,39,0.1)",
                      background: selectedDomain === domain.id ? domain.color + "18" : "#f5f6f8",
                    }}
                  >
                    <span style={{ fontSize: 26 }}>{domain.icon}</span>
                    <span style={{ fontSize: 12, color: selectedDomain === domain.id ? domain.color : "#4b5563", fontWeight: 600 }}>
                      {domain.id}
                    </span>
                  </button>
                ))}
                ...
                <div style={{ display: "flex", gap: 10 }}>
                  {DIFFICULTIES.map(d => (
                    <button
                      ...
                      style={{
                        ...styles.diffBtn,
                        background: selectedDifficulty === d ? (d === "Hard" ? "#ef4444" : d === "Medium" ? "#f59e0b" : "#f97316") + "20" : "#f5f6f8",
                        borderColor: selectedDifficulty === d ? (d === "Hard" ? "#ef4444" : d === "Medium" ? "#f59e0b" : "#f97316") : "#e2e6ee",
                        color: selectedDifficulty === d ? "#111827" : "#6b7280",
                      }}
                    >
                      {d === "Easy" ? "🟢" : d === "Medium" ? "🟡" : "🔴"} {d}
                    </button>
        ...
        {view === "test" && test && (
          <>
            ...
            {/* Progress bar */}
            <div style={{ height: 4, background: "#ffffff", borderRadius: 2, marginBottom: 24 }}>
              <div style={{ height: "100%", width: `${progress * 100}%`, background: "linear-gradient(90deg, #f59e0b, #f59e0b)", borderRadius: 2, transition: "width 0.3s" }} />
            </div>
              ...
              {test.questions.map((q, i) => (
                <button
                  ...
                  style={{
                    ...
                    background: answers[q._id] !== undefined ? "rgba(245,158,11,0.2)" : "#f5f6f8",
                    borderColor: i === currentQ ? "#f59e0b" : answers[q._id] !== undefined ? "rgba(245,158,11,0.4)" : "#e2e6ee",
                    color: i === currentQ ? "#f59e0b" : answers[q._id] !== undefined ? "#a5b4fc" : "#6b7280",
            ...
            {(() => {
              ...
              return (
                <div style={styles.questionCard}>
                  <h3 style={{ color: "#111827", fontSize: 18, lineHeight: 1.6, marginBottom: 24 }}>
                    Q{currentQ + 1}. {q.text}
                  </h3>
                  <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
                    {q.options.map((option, oi) => (
                      <button
                        ...
                        style={{
                          ...styles.optionBtn,
                          background: answers[q._id] === oi ? "rgba(245,158,11,0.15)" : "rgba(255,255,255,0.03)",
                          borderColor: answers[q._id] === oi ? "#f59e0b" : "#e2e6ee",
                          color: answers[q._id] === oi ? "#111827" : "#4b5563",
                        }}
                      >
                        <span style={{ ...styles.optionLetter, background: answers[q._id] === oi ? "#f59e0b" : "#ffffff" }}>
                          {["A","B","C","D"][oi]}
                        </span>
                        {option}
                      </button>
          ...
          <>
            ...
            <div style={styles.scoreCard}>
              <div style={{ textAlign: "center" }}>
                <svg width={120} height={120} viewBox="0 0 120 120">
                  <circle cx="60" cy="60" r="50" fill="none" stroke="#ffffff" strokeWidth="12" />
                  <circle
                    cx="60" cy="60" r="50"
                    fill="none"
                    stroke={result.score >= 70 ? "#f97316" : result.score >= 50 ? "#f59e0b" : "#ef4444"}
                    strokeWidth="12"
                    strokeDasharray={`${(result.score / 100) * 314} 314`}
                    strokeLinecap="round"
                    transform="rotate(-90 60 60)"
                  />
                  <text x="60" y="55" textAnchor="middle" fill="#111827" fontSize="26" fontWeight="bold">{result.score}</text>
                  <text x="60" y="76" textAnchor="middle" fill="#6b7280" fontSize="13">out of 100</text>
                </svg>
                <div style={{ marginTop: 10, fontWeight: 700, color: result.score >= 70 ? "#f97316" : result.score >= 50 ? "#f59e0b" : "#ef4444", fontSize: 18 }}>
              ...
              {result.skill_gaps?.length > 0 && (
                <div style={{ marginTop: 24, padding: 16, background: "#f5f6f8", borderRadius: 12, border: "1px solid rgba(239,68,68,0.2)" }}>
            ...
            <div style={{ marginTop: 24, display: "flex", flexDirection: "column", gap: 12 }}>
              {result.details?.map((d, i) => (
                <div key={i} style={{ ...styles.card, borderLeft: `4px solid ${d.is_correct ? "#f97316" : "#ef4444"}` }}>
                  <div style={{ display: "flex", gap: 12, alignItems: "flex-start" }}>
                    <span style={{ fontSize: 18, flexShrink: 0 }}>{d.is_correct ? "✅" : "❌"}</span>
                    <div style={{ flex: 1 }}>
                      <div style={{ fontSize: 14, color: "#111827", fontWeight: 500, marginBottom: 8 }}>Q{i + 1}. {d.text}</div>
                      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 8, marginBottom: 10 }}>
                        <div style={{ fontSize: 12, padding: "6px 10px", background: "rgba(245,158,11,0.1)", borderRadius: 6, color: "#a5b4fc" }}>
                          <strong>Your answer:</strong> {d.user_answer_text || "Skipped"}
                        </div>
                        <div style={{ fontSize: 12, padding: "6px 10px", background: "rgba(249,115,22,0.1)", borderRadius: 6, color: "#fdba74" }}>
                          <strong>Correct:</strong> {d.correct_answer_text}
                        </div>
                      </div>
                      {d.explanation && (
                        <div style={{ fontSize: 12, color: "#6b7280", padding: "8px 12px", background: "#f5f6f8", borderRadius: 6, lineHeight: 1.6 }}>
                          {d.explanation}
                        </div>
                      )}
                    </div>
            ...
            <div style={{ display: "flex", gap: 12, marginTop: 24 }}>
              ...
              <button onClick={() => navigate("/roadmap")} style={{ ...styles.generateBtn, background: "linear-gradient(135deg, #f97316, #ea580c)" }}>
...
const styles = {
  ...
  card: { background: "#ffffff", borderRadius: 14, padding: "20px", border: "1px solid rgba(17,24,39,0.06)" },
  cardTitle: { margin: "0 0 14px", fontSize: 15, fontWeight: 700, color: "#111827" },
  gapChip: { padding: "6px 12px", background: "#f5f6f8", borderRadius: 8, border: "1px solid", display: "flex", gap: 4, alignItems: "center" },
  ...
  generateBtn: { marginTop: 20, padding: "14px 28px", background: "linear-gradient(135deg, #f59e0b, #f59e0b)", border: "none", borderRadius: 10, color: "#fff", fontWeight: 700, fontSize: 15, cursor: "pointer", display: "block", width: "100%" },
  testHeader: { display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 20, flexWrap: "wrap", gap: 16 },
  timer: { padding: "10px 18px", background: "rgba(17,24,39,0.06)", borderRadius: 10, border: "1px solid", fontSize: 18, fontWeight: 700, fontFamily: "monospace" },
  submitBtn: { padding: "10px 20px", background: "linear-gradient(135deg, #f97316, #ea580c)", border: "none", borderRadius: 8, color: "#fff", fontWeight: 700, cursor: "pointer" },
  questionCard: { background: "#ffffff", borderRadius: 16, padding: "28px" },
  ...
  navBtn: { padding: "8px 18px", background: "#f5f6f8", border: "1px solid #e2e6ee", borderRadius: 8, color: "#6b7280", cursor: "pointer" },
  scoreCard: { background: "#ffffff", borderRadius: 16, padding: "32px", border: "1px solid rgba(17,24,39,0.06)", maxWidth: 500 },
};
