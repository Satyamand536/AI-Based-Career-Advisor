...
async function fetchFromTheMuse(limit = 50) {
  ...
  // Page over the most recent listings (descending) and keep jobs whose Muse
  // taxonomy category is tech-related. This returns real, currently-open jobs
  // and avoids the noisy mixed feed (retail, healthcare, etc.).
  const results = [];
  const pages = Math.ceil(limit / 20) + 2;
  for (let page = 1; page <= pages; page++) {
    try {
      const params = { page, descending: true, api_key: API_KEY };
      // The Muse allows unauthenticated (rate limited) — only send key if provided
      if (!API_KEY) delete params.api_key;
      ...
      const jobs = (res.data?.results || [])
        .filter(job => (job.categories || []).some(c => TECH_MUSE_CATEGORIES.includes(c.name)))
        .map(job => ({
          title: job.name || "",
          ...
          company_logo: job.company?.refs?.logo_image || "",
        }));
      results.push(...jobs);
    } catch (err) {
      console.error(`[JobFetcher] The Muse error on page ${page}:`, err.message);
    }
  }
  return results;
}
