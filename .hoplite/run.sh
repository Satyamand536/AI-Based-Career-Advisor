...
# The keeper self-locks (.keeper.lock), so this only starts it when absent.
if [ ! -f "$HOPLITE/.keeper.lock" ] || ! kill -0 "$(cat "$HOPLITE/.keeper.lock")" 2>/dev/null; then
  setsid nohup bash "$HOPLITE/stack_keeper.sh" >> /tmp/keeper.log 2>&1 &
  echo "[run] stack keeper armed"
else
